...
lemma Nat.mul_succ (n m:Nat) : n * m++ = n * m + n := by
  ...
  . intro n ih
    ...
    grind [add_comm, add_assoc, succ_add_eq_add_succ]
...
lemma Nat.pos_mul_pos {n m: Nat} (h₁: n.IsPos) (h₂: m.IsPos) : (n * m).IsPos := by
  obtain ⟨a, ha⟩ := (uniq_succ_eq n h₁).exists
  rw [<-ha, succ_mul]
...
lemma Nat.mul_eq_zero (n m: Nat) : n * m = 0 ↔ n = 0 ∨ m = 0 := by
  ...
  . contrapose
    push_neg
    repeat rw [<-isPos_iff]
    grind only [pos_mul_pos]
  . intro h
    obtain hn | hm := h
    . subst hn; exact zero_mul m
    . subst hm; exact mul_zero n
...
/-- This illustration of the `ring` tactic is not from the
    textbook. -/
example (a b c d:ℕ) : (a+b)*1*(c+d) = d*b+a*c+c*b+a*d := by ring
...
lemma Nat.mul_le_mul_left {a b c: Nat} (h: a ≤ b) : c * a ≤ c * b := by
  obtain heq | hlt := le_iff_eq_or_lt.mp h
  . subst heq
    exact le_refl _
  . if hc: c = 0 then
      simp only [hc, zero_mul, le_refl]
    else
      exact mul_lt_mul_of_pos_left hlt hc |> le_of_lt

instance Nat.isOrderedRing : IsOrderedRing Nat where
  zero_le_one := zero_le 1
  mul_le_mul_of_nonneg_left := by grind only [mul_le_mul_left]
  mul_le_mul_of_nonneg_right := by grind only [mul_le_mul_left, mul_comm]
...
theorem Nat.exists_div_mod (n:Nat) {q: Nat} (hq: q.IsPos) :
  ...
  . use 0, 0
    ...
    . exact le_refl 0
    . exact zero_le q
    . exact isPos_iff q |>.mp hq |>.symm
    . rw [zero_mul, zero_add]
  . intro n ih
    choose m r h using ih
    let ⟨hr0, hrq, hn⟩ := h
    have : r++ ≤ q := by simp_all [lt_iff_succ_le]
    obtain heq | hlt := le_iff_eq_or_lt.mp this
    . use m++, 0
      split_ands
      . exact le_refl 0
      . exact zero_le q
      . exact isPos_iff q |>.mp hq |>.symm
      . rw [hn, <-add_succ, heq, succ_mul, add_zero]
    . use m, (r++)
      split_ands
      . exact zero_le _
      . exact le_of_lt hlt
      . exact ne_of_lt _ _ hlt
      . rw [add_succ, hn]
...
lemma Nat.two_mul' (m: Nat) : 2 * m = m + m := by grind [two_mul, add_zero]
theorem Nat.sq_add_eq (a b: Nat) :
    (a + b) ^ (2 : Nat) = a ^ (2 : Nat) + 2 * a * b + b ^ (2 : Nat) := by
  rw [pow_two, mul_add, add_mul, add_mul, <-pow_two, <-pow_two, mul_comm, add_assoc, <-add_assoc (a*b), <-two_mul']
